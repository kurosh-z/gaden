/*-------------------------------------------------------------------------------
 * This node simulates the response of a MOX gas sensor given the GT gas
 * concentration of the gases it is exposed to (request to simulation_player or
 * dispersion_simulation)
 * - Gas concentration should be given in [ppm]
 * - The Pkg response can be set to:  Resistance of the sensor (Rs),
 * Resistance-ratio (Rs/R0), or Voltage (0-5V)
...
int main(int argc, char **argv) {
  ...
  // Publishers
  ros::Publisher sensor_read_pub =
      n.advertise<olfaction_msgs::gas_sensor>("Sensor_reading", 500);
  ros::Publisher marker_pub =
      n.advertise<visualization_msgs::Marker>("Sensor_display", 100);

  // Service to request gas concentration
  ros::ServiceClient client =
      n.serviceClient<gaden_player::GasPosition>("/odor_value");
    ...
    if (know_sensor_pose) {
      ...
      if (client.call(srv)) {
        /*
                        ...
                        {
                            ROS_INFO("[FakeMOX] %s:%.4f at
           (%.2f,%.2f,%.2f)",srv.response.gas_type[i].c_str(),
           srv.response.gas_conc[i],srv.request.x, srv.request.y, srv.request.z
           );
                        }

        */

        // Simulate Gas_Sensor response given this GT values of the
        // concentration!
        ...
        case 0: // MOX TGS2620
          ...
          sensor_msg.raw_air =
              Sensitivity_Air[input_sensor_model] * R0[input_sensor_model];
          sensor_msg.calib_A = sensitivity_lineloglog[input_sensor_model][0]
                                                     [0]; // Calib for Ethanol
          sensor_msg.calib_B = sensitivity_lineloglog[input_sensor_model][0]
                                                     [1]; // Calib for Ethanol
          break;
        case 1: // MOX TGS2600
          sensor_msg.technology = sensor_msg.TECH_MOX;
        ...
        if (!notified) {
          ROS_WARN("[fake_gas_sensor] Cannot read Gas Concentrations from "
                   "simulator.");
          notified = true;
        }
      }
...
// RS = R0*( A * conc^B )
// This method employes a curve fitting based on a line in the loglog scale to
// set the sensitivity
float simulate_mox_as_line_loglog(
    gaden_player::GasPositionResponse GT_gas_concentrations) {
  if (first_reading) {
    ...
    for (int i = 0; i < GT_gas_concentrations.positions[0].concentration.size();
         i++) {
      int gas_id;
      if (!strcmp(GT_gas_concentrations.gas_type[i].c_str(), "ethanol"))
        ...
        ROS_ERROR(
            "[fake_mox] MOX response is not configured for this gas type!");
        return 0.0;
      }
      ...
      */
      ...
      RS_R0 = sensitivity_lineloglog[input_sensor_model][gas_id][0] *
              pow(GT_gas_concentrations.positions[0].concentration[i],
                  sensitivity_lineloglog[input_sensor_model][gas_id][1]);
  ...
  // Return Sensor response for current time instant as the Sensor Resistance in
  // Ohms
  ...
  for (int i = 0; i < GT_gas_concentrations.positions[0].concentration.size();
    ...
    if (use_PID_correction_factors) {
      ...
      if (!strcmp(GT_gas_concentrations.gas_type[i].c_str(), "ethanol"))
        ...
        ROS_ERROR(
            "[fake_PID] PID response is not configured for this gas type!");
        return 0.0;
      }
      if (PID_correction_factors[gas_id] != 0)
        accumulated_conc +=
            GT_gas_concentrations.positions[0].concentration[i] /
            PID_correction_factors[gas_id];
    } else
...
void loadNodeParameters(ros::NodeHandle private_nh) {
  ...
  // Sensor Model
  private_nh.param<int>("sensor_model", input_sensor_model,
                        DEFAULT_SENSOR_MODEL);

  // sensor_frame
  private_nh.param<std::string>("sensor_frame", input_sensor_frame,
                                DEFAULT_SENSOR_FRAME);

  // PID_correction_factors
  private_nh.param<bool>("use_PID_correction_factors",
